...
The robot code is organized by responsibility so OpModes stay small and hardware/control logic
...
| --- | --- |
| `config` | Hardware-map names and tunable robot-wide values/variables |
...
(Do NOT change the configuration throughout the season omg. Make it and STICK TO IT. )
...
| `servoMotor` | Continuous-rotation servo (spatula) | Auxiliary servo mechanism | Controlled by B |
...
| B | Toggle continuous-rotation (spatula) servo on/off |
